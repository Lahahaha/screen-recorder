...
- 🧩 **多屏截图与合成**：默认自动检测屏幕数量，多屏时保存同一批次截图并生成合成视频。
...
| ▶ 开始 / ⏸ 暂停 | 开始或暂停定时截屏 |
| ⏱ 间隔 | 设置截屏间隔 |
| 🖥️ 截屏范围 | 自动多屏，或固定只截某一块屏幕 |
| 🎬 生成今日视频 | 将今天的截图生成视频 |
| 历史视频 | 打开历史视频窗口 |
| 📁 打开保存目录 | 打开截图、视频和配置所在目录 |
| 工作目录... | 管理历史工作目录、新增、切换、打开或删除目录 |
...
- 支持添加外部文件夹。
- 支持多选并逐个生成视频。
- 生成模式可选择多屏合成，或只使用 `screen-01`、`screen-02` 等单屏截图。
...
工作目录窗口用于管理截图、视频、配置和日志所在的根目录。

- 支持新增已有目录或空目录；已有 `config.json`、`screenshots`、`videos` 会直接沿用，不会覆盖配置。
- 新增目录会按规范化绝对路径去重；列表中已有时只提示，不重复添加。
- 切换工作目录会保存为“下次启动使用”，当前运行中的截图、视频生成、日志和配置仍写入当前目录。
- 删除目录时可选择移到回收站、永久删除或取消；删除后历史记录会保留，恢复到原路径后会自动变回可用。
- “移出列表”只删除历史记录，不删除磁盘文件；当前目录和下次启动目录不能移出列表。
...
用户选择的工作目录保存在系统配置目录中的 `workdirs.json` 指针文件里，不在可切换的工作目录内。工作目录本身仍保持以下结构：
...
| `capture_mode` | `"auto"` | 截屏范围，支持 `"auto"`、`"screen-01"`、`"screen-02"` 等 |

## 视频生成规则

- 自动模式下，单屏机器继续保存旧格式文件：`HH-MM-SS.mmm-000123.png` 或 `.jpg`。
- 自动模式下，多屏机器会为同一时间点保存多张图片：`HH-MM-SS.mmm-screen-01-000123.png`、`screen-02` 等，并写入 `.screens.json`。
- 用户也可以在托盘中固定只截某一块屏幕；这种模式继续使用旧单屏文件名。
- 多屏批次中某块屏幕截图失败时，成功屏幕仍会保存；metadata 只记录成功屏幕。
- 视频生成会忽略非图片文件。
- 外部非标准命名图片也可以作为普通单图帧参与生成。
- 损坏图片会被跳过；如果没有任何可读图片，则生成失败并显示错误。
- 单屏视频保持原图尺寸，必要时将宽高调整为偶数以兼容视频编码。
- 多屏批次文件会按同一时间点和 sequence 分组，并优先使用 `.screens.json` 中的几何信息合成。
- 历史视频窗口可以切换为单屏生成；单屏输出会追加 `-screen-XX` 后缀。
